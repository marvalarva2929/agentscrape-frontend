...
function App() {
  ...
  return <AppShell onLogout={handleLogout} onNavigateSchools={() => setScreen('main')} onNavigateHistory={() => setScreen('history')} onNavigateGame={() => setScreen('game')} onNavigateCrawl={() => { void navigateToCrawl() }} statusText="Backend online">
    ...
    {screen === 'crawl' && (() => {
      ...
      return <main className="page-shell narrow-shell"><div className="page-header-row"><div><div className="breadcrumb">Run Crawl</div><h2>Run Crawl</h2></div></div><section className="panel-block"><div className="field-group"><label className="input-label">School</label><select value={wizardDraft.schoolId} onChange={(event) => { const school = schools.find((item) => item.id === event.target.value); setWizardDraft((current) => ({ ...current, schoolId: event.target.value, schoolUrl: school?.canonicalUrl ?? '', includeDirectory: false })) }}><option value="">New school — enter URL below</option>{schools.map((school) => <option key={school.id} value={school.id}>{school.name}</option>)}</select></div><div className="field-group"><label className="input-label">School URL</label><input value={wizardDraft.schoolUrl} onChange={(event) => setWizardDraft((current) => ({ ...current, schoolUrl: event.target.value, includeDirectory: false }))} placeholder="https://school.edu" /></div><div className="field-group"><label className="input-label">Operations</label><label className="checkbox-row"><input type="checkbox" checked readOnly /><span>Crawl</span></label>{crawlSchool?.directoryUrl && <label className="checkbox-row"><input type="checkbox" checked={wizardDraft.includeDirectory} onChange={(event) => setWizardDraft((current) => ({ ...current, includeDirectory: event.target.checked }))} /><span>Directory Search</span></label>}</div><div className="field-group"><label className="input-label">Budget (USD)</label><input type="number" min={1} value={wizardDraft.maxSpendUsd} onChange={(event) => setWizardDraft((current) => ({ ...current, maxSpendUsd: event.target.value }))} placeholder="10" /></div><label className="checkbox-row"><input type="checkbox" checked={wizardDraft.forceRescan} onChange={(event) => setWizardDraft((current) => ({ ...current, forceRescan: event.target.checked }))} /><span>Force rescan</span></label><div className="review-box"><div className="review-row"><span>Operation</span><strong>{wizardDraft.includeDirectory && crawlSchool?.directoryUrl ? 'Crawl + Directory Search' : 'Crawl'}</strong></div><div className="review-row"><span>School</span><strong>{crawlSchool?.name ?? wizardDraft.schoolUrl ?? '—'}</strong></div></div>{wizardError && <div className="error-banner">{wizardError}</div>}<div className="modal-actions"><button className="secondary-button" onClick={() => setScreen('main')}>Cancel</button><button className="primary-button" onClick={() => void handleSubmitWizard()}>Start Crawl</button></div></section></main>
    })()}
  </AppShell>
}
