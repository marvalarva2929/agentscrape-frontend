...
export type Screen = 'main' | 'person' | 'run-monitor' | 'crawl' | 'history'
...
function App() {
  ...
  const handleSubmitWizard = async () => {
    ...
    try {
      setWizardError('')
      const job = await runsApi.startRun({ schoolId: wizardDraft.schoolId, schoolUrl, maxSpendUsd: budget, forceRescan: wizardDraft.forceRescan })
      const school = schools.find((item) => item.id === wizardDraft.schoolId)
      setRun({ ...job, schoolId: wizardDraft.schoolId || undefined, schoolName: school?.name ?? hostOf(schoolUrl), runType: 'New Crawl' })
  ...
  return <AppShell onLogout={handleLogout} onNavigateSchools={() => setScreen('main')} onNavigateHistory={() => setScreen('history')} onNavigateCrawl={() => { void navigateToCrawl() }} statusText="Backend online">
